"""
Vue / Nuxt Agentic Creator v1.2
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Fixed version — mirrors the nextjs_creator v13.1 architecture exactly.
Socket-ready · Self-learning cache · Auto-updater · Supreme power.

═══════════════════════════════════════════════════════
FIXES in v1.2 (on top of v1.1)
═══════════════════════════════════════════════════════

FIX-A  _ver() HANGS ON WINDOWS (BUN / PNPM / YARN)
       subprocess.run(capture_output=True, shell=True, timeout=N) on
       Python 3.10 / Windows can deadlock because the stdout reader
       thread blocks even after the timeout fires.
       FIX → On Windows, pass creationflags=CREATE_NO_WINDOW so the
       child process doesn't inherit the console; also fall back to
       communicate(timeout=…) inside a try/except TimeoutExpired so the
       process is forcibly killed and the call always returns.

FIX-B  NUXT SCAFFOLD IS INTERACTIVE → "All strategies exhausted"
       Neither `npm create nuxt@latest` nor `npx nuxi@latest init`
       suppresses all interactive prompts by default; both pause waiting
       for user input (package manager, git init, etc.) in a non-TTY
       environment.
       FIX → _build_nuxt_strategies() now always appends
         --packageManager <pm>   (tells nuxi which PM to use)
         --no-install            (skip the auto-install so we can control it)
         --gitInit false         (suppress git prompt — only when not already set)
       A separate install step is added in _run_pipeline() for nuxt so
       dependencies are installed after scaffold even with --no-install.

FIX-C  INSTALL DIR NOT CREATED IN init BRANCH
       os.makedirs(install_dir, exist_ok=True) was only called in the
       ask_for_path branch; the init branch extracted the path but never
       created it before handing it to _run_pipeline(), causing the
       scaffold command to fail with "directory not found".
       FIX → os.makedirs call added to the init branch immediately after
       path extraction succeeds.

FIX-D  vite-plugin-vue-devtools NOT INSTALLED
       When create-vue scaffolds with --default it adds
       vite-plugin-vue-devtools to vite.config.js but the package is
       sometimes absent from node_modules (peer-dep resolution failure).
       FIX → after scaffold succeeds, _run_pipeline() runs a bare
       `<pm> install` in the project directory to ensure all deps are
       present before the dev server health check.

FIX-E  NUXT PROJECT NAME SWALLOWS PATH WORDS
       The regex path stripper sometimes left "test-in-c-vrive" as the
       project name because the path phrase wasn't fully removed before
       the AI saw the prompt.  The strip patterns are extended to also
       match "in <letter> drive/vrive" (common voice-recognition typos).
...
try:
    ...
    def _terminal_cb(text="", color="white", msg_type="normal", data=None):
        ...
        if msg_type == "ask_path":
            ...
            if data:
                _rc.print(f"[dim]  keys: {list(data.keys())}[/dim]")
            return
...
SELF_UPDATE_URL = ""   # Set to raw GitHub URL of latest release to enable
...
HEALTH_POLL    = 2
MAX_FIX_ROUNDS = 3
KNOWLEDGE_TTL  = 86400   # 24 h in seconds

sys.path.insert(0, _HERE)
try:
    from experiment.test3.project_fixer import ProjectFixer, fix_project
    HAS_FIXER = True
except ImportError:
    HAS_FIXER = False
...
_DEFAULT_KNOWLEDGE = {
    ...
    "nuxt_flags": {
        ...
        "--template":        "Specify starter template or GitHub repo",
    ...
    "nuxt_create_cmds": {
        ...
        "npx_nuxi":   "npx nuxi@latest init",
        "npx_create": "npx create-nuxt-app",
    },
...
def refresh_knowledge(k: dict, cb: PrintCallback, force: bool = False):
    try:
        ...
        _refresh_vue_flags(k, cb)
        _refresh_nuxt_flags(k, cb)
        k["_fetched_at"] = now
        _save_knowledge(k)
...
def _refresh_vue_flags(k: dict, cb: PrintCallback):
    try:
        status, text = _http_get(
            "https://raw.githubusercontent.com/vuejs/create-vue/main/index.ts", timeout=8)
        if status != 200:
            return
        for flag in re.findall(r"'(--[a-z][a-z0-9-]+)'", text):
            if flag not in k["vue_flags"]:
                k["vue_flags"][flag] = "discovered automatically"
                cb(f"  🆕 Vue flag discovered: {flag}", "magenta", "normal", {})
    except Exception:
        pass


def _refresh_nuxt_flags(k: dict, cb: PrintCallback):
    try:
        status, text = _http_get(
            "https://raw.githubusercontent.com/nuxt/cli/main/src/commands/init.ts", timeout=8)
        if status != 200:
            return
        for flag in re.findall(r"'(--[a-z][a-z0-9-]+)'", text):
            if flag not in k["nuxt_flags"]:
                k["nuxt_flags"][flag] = "discovered automatically"
                cb(f"  🆕 Nuxt flag discovered: {flag}", "magenta", "normal", {})
    except Exception:
        pass
...
def check_self_update(cb: PrintCallback):
    ...
    try:
        ...
        with open(staging, "w", encoding="utf-8") as f:
            f.write(remote_src)
        cb(
            f"New version available! Staged at {staging} — restart to apply.",
            "magenta", "self_update",
            {"staging": staging, "remote_sha": remote_sha, "local_sha": local_sha},
        )
    except Exception:
        pass
...
# FIX-E: extended to catch "c vrive", "c drive", "c disk" voice-recognition variants
_NL_PATH_PATTERNS = [
    # "in c drive", "in c vrive", "in the c drive", "in c disk"
    (r'\b(?:in|on|at|inside)\s+(?:the\s+)?([A-Za-z])\s+(?:drive|vrive|disk|partition)\b',
     lambda m: m.group(1).upper() + ":\\"),
    # bare drive letter  C:\  or  C:/
    (r'\b([A-Za-z]):[\\\/]',
     lambda m: m.group(1).upper() + ":\\"),
]
...
_TYPO_MAP = {
    ...
    "vrive":  "drive",   # voice-recognition typo
}
...
# FIX-E: extended strip patterns to remove "in <letter> drive/vrive" variants
_PATH_STRIP_PATTERNS = [
    ...
    r'(?<!\w)~[^\s,;\"\']+',
    r'(?:path|dir(?:ectory)?|folder)\s*[=:]\s*[^\s,;\"\']+',
    # Catch leftover "in <letter> drive/vrive" after path removal
    r'\bin\s+[A-Za-z]\s+(?:drive|vrive|disk|partition)\b',
    # Catch leftover bare location prepositions
...
# FIX-A: _ver() no longer hangs on Windows.
# On Windows, CREATE_NO_WINDOW prevents console-inheritance deadlocks.
# We use Popen + communicate(timeout) so the process is force-killed on timeout.
...
# ─────────────────────────────────────────────────────────────────────────────
# ══════════════════  KNOWLEDGE-BACKED CACHE HELPERS  ═════════════════════════
# ─────────────────────────────────────────────────────────────────────────────
...
# FIX-B: _build_nuxt_strategies always appends non-interactive flags so nuxi
# never blocks waiting for user input in a non-TTY/pipe environment.
def _build_nuxt_strategies(intent: Intent, k: dict) -> list:
    """
    Returns list of (strategy_name, full_cmd).

    FIX-B: Appends --packageManager <pm> --no-install to every strategy so
    nuxi init never prompts interactively.  --gitInit false is appended only
    when the user did not explicitly request --gitInit.

    IMPORTANT: because --no-install is set, _run_pipeline() must run a
    separate `npm install` (or equivalent) after the scaffold succeeds.
    """
    result = []
    pm     = intent.pm
    name   = intent.project_name

    # Build non-interactive flag set
    flags = list(intent.flags)

    # Only add --packageManager if user didn't specify it already
    if not any("--packageManager" in f for f in flags):
        flags.append(f"--packageManager {pm}")

    # --no-install: we'll run install ourselves (FIX-D dependency)
    if "--no-install" not in flags:
        flags.append("--no-install")

    # Suppress git prompt if user didn't ask for it
    if "--gitInit" not in flags:
        flags.append("--gitInit false")
    ...
    cmds = k.get("nuxt_create_cmds", _DEFAULT_KNOWLEDGE["nuxt_create_cmds"])
    add(f"default_nuxt_{pm}", cmds.get(pm, cmds["npm"]))
    add("npx_nuxi_latest",   "npx nuxi@latest init")
    add("npx_create_nuxt",   "npx create-nuxt-app")
    return result
        ...
        for fix_round in range(MAX_FIX_ROUNDS + 1):
            ...
            for sname, cmd in strats:
                ...
                if ok or ek == EK.SOFT_SUCCESS:
                    ...
                    if sname.startswith("cached_") or "default_" in sname:
                ...
                self._cb(f"  ✗ Failed ({ek}): {(err + out).strip()[:200]}", "red", "error", {})
...
PATH RULE: All path/directory references have already been removed.
Ignore any leftover location words (drive, folder, directory, test, vrive, c).
Extract ONLY project properties.

DO NOT add --packageManager, --no-install, or --gitInit — the runner
adds these automatically to prevent interactive prompts.

VALID NUXT FLAGS (only include what the user explicitly asked for):
  --nightly   --template <name>   --force
  --modules <module1,module2>

NUXT MODULES the user may mention (put in "libraries" if mentioned):
...
def _run_pipeline(
    ...
    """
    Scaffold project → install deps → install libs → verify dev server.
    Returns (healthy: bool, intent: Intent | None).

    FIX-D: After scaffold, always run a bare `<pm> install` in the project
    directory.  This ensures vite-plugin-vue-devtools and any other peer
    dependencies that the scaffold wrote into package.json but didn't install
    (because --no-install was passed for nuxt, or peer-dep resolution failed
    for vue) are present before the dev server health check.
    """
    ...
    cb(f"  Flags       : {intent.flag_str() or 'none'}",               "cyan", "normal", {})
    ...
    # ── FIX-D: bare install to resolve all deps (incl. vite-plugin-vue-devtools)
...
def start(
    ...
    cb("", "white", "normal", {})
    cb("⚡ Vue / Nuxt Agentic Creator v1.2", "cyan", "normal", {})
    cb("   Self-Learning · Auto-Updater · Retry Loops · Socket-Ready", "cyan", "normal", {})
    ...
    # ── STEP 4: parse project properties ─────────────────────────────────────
    cb("", "white", "normal", {})
    ...
    # ── STEP 5a: NO path → ask immediately ───────────────────────────────────
    if not install_dir:
        ask_payload = {
            "task_id":      task_id,
    ...
    # ── STEP 5b: path found → refresh knowledge then run pipeline ────────────
    refresh_knowledge(k, cb)
